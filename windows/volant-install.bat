...
set "SHORTCUT_PATH=%USERPROFILE%\Desktop\Volant.lnk"
...
powershell -ExecutionPolicy Bypass -Command "$s=(New-Object -COM WScript.Shell).CreateShortcut('%SHORTCUT_PATH%'); $s.TargetPath='%TARGET_BAT%'; $s.WorkingDirectory='%CD%\%VOLANT_SOURCE%'; if (Test-Path '%ICON_PATH%') { $s.IconLocation='%ICON_PATH%,0' } else { $s.IconLocation='C:\Windows\System32\shell32.dll, 22' }; $s.Description='Launch Volant fly tracking analysis'; $s.Save()"
